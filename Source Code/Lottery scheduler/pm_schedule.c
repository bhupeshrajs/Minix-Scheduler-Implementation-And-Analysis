...
/* nice_to_priority() function is bypassed so that original nice value is not lost*/
//		if ((rv = nice_to_priority(nice, &maxprio)) != OK) {
//			return rv;
//		}

		m.SCHEDULING_MAXPRIO	= nice;			//nice is the value of parameter given to nice() command in the test file
